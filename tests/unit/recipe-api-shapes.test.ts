...
describe("recipe API request schemas", () => {
  ...
  it("validates the explicit favorite target and returns a uniform not-found envelope", async () => {
    const invalidId = await setFavorite(new Request("http://localhost/api/recipes/x/favorite", { method: "PATCH", body: JSON.stringify({ isFavorite: true }) }), { params: { id: "x" } });
    expect(invalidId.status).toBe(400);
    expect(await invalidId.json()).toMatchObject({ error: { code: "invalid_id" } });

    const invalidBody = await setFavorite(new Request("http://localhost/api/recipes/7/favorite", { method: "PATCH", body: JSON.stringify({ isFavorite: "true" }) }), { params: { id: "7" } });
    expect(invalidBody.status).toBe(400);
    expect(await invalidBody.json()).toMatchObject({ error: { code: "invalid_request" } });

    state.setFavorite.mockReturnValue(false);
    const missing = await setFavorite(new Request("http://localhost/api/recipes/7/favorite", { method: "PATCH", body: JSON.stringify({ isFavorite: true }) }), { params: { id: "7" } });
  ...
  it("persists the requested favorite target instead of toggling server state", async () => {
    ...
    const response = await setFavorite(new Request("http://localhost/api/recipes/7/favorite", { method: "PATCH", body: JSON.stringify({ isFavorite: true }) }), { params: { id: "7" } });
