...
type ImageCarouselProps = { images: string[]; showDelete?: boolean; onDelete?: (url: string) => void; selectedUrls?: string[]; coverUrl?: string | null; onToggleSelection?: (url: string) => void; onSetCover?: (url: string) => void; variant?: "default" | "detailHero" | "imageReview"; reviewLabel?: string };

export function ImageCarousel({ images, showDelete, onDelete, selectedUrls, coverUrl, onToggleSelection, onSetCover, variant = "default", reviewLabel }: ImageCarouselProps) {
  ...
  return <div className={frameClass} data-testid={review ? "image-review-carousel" : undefined}>
    <div className={variant === "detailHero" ? "recipe-detail-hero-frame" : review ? "image-review-main-frame" : "relative overflow-hidden rounded-lg"}>
      ...
      {review ? (coverUrl === currentUrl ? <span className="image-review-cover-pill">封面图</span> : <Button className="image-review-cover-pill" aria-label="设为封面" onClick={() => onSetCover?.(currentUrl)}>设为封面</Button>) : filtered.length > 1 ? <><Button variant="secondary" size="icon" aria-label="上一张图片" className="absolute left-2 top-1/2 -translate-y-1/2" onClick={previous}><ChevronLeft /></Button><Button variant="secondary" size="icon" aria-label="下一张图片" className="absolute right-2 top-1/2 -translate-y-1/2" onClick={next}><ChevronRight /></Button></> : null}
    </div>
    {review ? <>{reviewLabel ? <p className="image-review-all-count">{reviewLabel}</p> : null}<div className="image-review-thumbnails" data-testid="image-review-thumbnails">{filtered.map((url, imageIndex) => { const isSelected = selectedUrls?.includes(url) ?? true; return <Button key={url} variant="ghost" aria-label={`${isSelected ? "取消选择" : "选择"}第 ${imageIndex + 1} 张图片`} aria-pressed={isSelected} data-testid="image-review-thumbnail" className={`image-review-thumbnail ${isSelected ? "is-selected" : "is-muted"} ${coverUrl === url ? "is-cover" : ""}`} onClick={() => { setIndex(imageIndex); onToggleSelection?.(url); }}><img src={url} alt="" className="h-full w-full object-cover" />{isSelected ? <Check className="image-review-thumbnail-check" /> : null}</Button>; })}</div></> : null}
