...
const GUIDE_FALLBACK_IMAGE = "/stitch-v3/stitch-image-20.jpg";

function CookingGuideImage({ recipe }: { recipe: RecipeDetail }) {
  const [source, setSource] = useState<string | null>(recipe.coverImageUrl ?? recipe.imageUrls[0] ?? null);

  if (!source) return <div className="cooking-guide-image cooking-guide-image-fallback" role="img" aria-label={`${recipe.name} 成品图`}>{recipe.name}</div>;

  return <img className="cooking-guide-image" src={source} alt={`${recipe.name} 成品图`} onError={() => setSource((current) => current === GUIDE_FALLBACK_IMAGE ? null : GUIDE_FALLBACK_IMAGE)} />;
}
